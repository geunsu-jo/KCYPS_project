...
# 식별ID로 데이터 병합
## 자녀 데이터는 있으나 부모 데이터는 누락된 경우 데이터 제거
## 자녀 데이터에 대응하는 부모 데이터가 2개인 경우(ex 엄빠가 모두 조사에 참여한 경우), 2개의 데이터로 취급
...
select_feature_lasso <- function(code, train_data, test_data){
  ...
  # LASSO
  cv_m <- cv.glmnet(train_x, factor(train_y), alpha=1, type.measure = "auc",
...
############## one-standard error rule 예시 시각화 자료 ####################################

idx_code <- str_detect(colnames(train_data), "YTIM")
train_x_raw <- train_data %>% select(colnames(train_data)[idx_code]) %>% as.matrix()
train_x <- scale(train_x_raw, scale=T)
...
se <- cv_m$cvsd
avg <- cv_m$cvm
AUC <- cbind(avg - se, avg, avg + se)
matplot(60:81, AUC[60:81,], type="l", col=c(1,2,1), lty=c(3,1,3),
        xlab="lambda index(역순)", ylab="Cross-Validation Error",
        ylim=c(min(AUC), max(AUC)), cex.lab=1.8)
matplot(60:which.max(AUC[,2]), rep(max(AUC[,1]),15), 
        type="l", col=1, lty=3, add=T)
points(which.max(AUC[,2]), max(AUC[,1]), pch=19, col="black")

points(60:81, AUC[60:81,2], pch=19, col="black")
up <- which(AUC[,2] > AUC[which.max(AUC[,2]),1])
points(min(up), AUC[min(up),2], pch="x", col="blue", cex=2.3)
text(min(up)-2.8, AUC[min(up),2]+0.003, "One-standard error rule", cex=2.1)

points(which.max(AUC[,2]), max(AUC[,2]), pch="x", col="blue", cex=2.3)
text(which.max(AUC[,2])-2.5, max(AUC[,2])+0.005, "5-fold Maximize AUC", cex=2.1)
