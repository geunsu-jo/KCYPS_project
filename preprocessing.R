...
# 결측치가 존재하는 컬럼 삭제
na_idx <- unique(which(is.na(data_raw2), arr.ind = T)[,2]) # 결측치 존재하는 컬럼 인덱스 찾기
...
#########################
# 반응변수(비행) 중영역별로 묶기 (비행변수 코드 "YDLQ")
idx_y <- str_detect(feature_name, "YDLQ")
y_raw <- data %>% select(feature_name[idx_y])

# 한번이라도 비행 경험이 있으면 1, 없으면 0
y_sum <- apply(y_raw, 1, sum)
y <- ifelse(y_sum==30, 1, 0)
...
set.seed(42)
foldid <- sample(1:5, nrow(train_YTIM), replace=T)
# Hyperparameter
lam <- 10^seq(5, -3, -0.1)

select_feature_lasso <- function(code_name, train_data, test_data){
  feature <- colnames(train_data)
  idx <- str_detect(feature, code_name)
  train_x <- train_data %>% select(feature[idx]) %>% as.matrix()
  
  cv_m <- cv.glmnet(train_x, factor(train_y), alpha=1, 
                    foldid=foldid, lambda=lam, family="binomial")
  
  coefs <- coef(cv_m, s="lambda.min") # one-standard error rule 적용
...
cor(data.frame(train_YPHY, train_y))
...
train_x <- data.frame(train_YTIM, train_YINT, train_YFUR, 
                      train_YPSY, train_YPHY, train_YACT, 
                      train_YEDU, train_YFAM, train_PPSY, 
                      train_PPHY)

test_x <- data.frame(test_YTIM, test_YINT, test_YFUR, 
                      test_YPSY, test_YPHY, test_YACT, 
                      test_YEDU, test_YFAM, test_PPSY, 
                      test_PPHY)
